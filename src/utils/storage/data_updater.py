...
class DataUpdater(BaseStorageHandler):
    ...
    def update_season_data(self, season_data: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            for i, existing_season in enumerate(current_data["seasons"]):
                if (existing_season["season"] == season_data["season"] and 
                    existing_season["year"] == season_data["year"]):
            ...
            self.logger.info(f"Updated season: {season_data.get('season')} {season_data.get('year')}")
            return True
    ...
    def update_designer_data(
        ...
        try:
            ...
            season["completed_designers"] = sum(
                1 for d in season["designers"] if d["completed"]
            )
            season["completed"] = (
                season["completed_designers"] >= season["total_designers"]
            )
    ...
    def update_look_data(
        ...
        try:
            ...
            self.write_data(current_data)
            self.logger.info(
                f"Updated look {look_number} with {len(images)} new images"
            )
            return True
    ...
    def _update_completion_status(
        ...
        designer["extracted_looks"] = sum(
            1 for look in designer["looks"] if look["completed"]
        )
        designer["completed"] = (
            designer["extracted_looks"] >= designer["total_looks"]
        )
